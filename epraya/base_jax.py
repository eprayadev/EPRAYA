...
def Computejacobian(func,pflat,unrav,Ham,Exp,expr,mode,iwas,jwas,kwas,weight,hulk):
    pflatj=jxn.array(pflat)
    nparam=len(pflatj)
    @jx.jit
    def Jacolumn(pflat_,tangent_):
        _,col=jx.jvp(lambda p: func(p,unrav,Ham,Exp,expr,mode,iwas,jwas,kwas,weight,hulk),(pflat_,),(tangent_,))
        return col
    cols=[]
    for i in range(nparam):
        tangent=jxn.zeros_like(pflatj).at[i].set(1.0)
        col=Jacolumn(pflatj,tangent)
...
def Briggs(Hamer,Exp,Vary,expr,maximal=2000,eps=1e-11,mode='p',M=70):
    ...
    if isinstance(Hamer,JHval):
      ...
      try:
          ...
          if fitres.paramerrorsdict is not None:
